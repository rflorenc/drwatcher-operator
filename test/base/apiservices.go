		...
		for _, condition := range s.Status.Conditions {
			if condition.Type == apiregv1.Available {
				if condition.Status == apiregv1.ConditionTrue {
					logger.Debug("API service is ready")
					return true
				}
			}
		}
		logger.Debug("API service is not ready yet")
		return false
	}
...
func WaitForAPIService(c apiregv1client.ApiregistrationV1Interface, name string, testFunc func(*apiregv1.APIService) bool, timeOut time.Duration) error {
	logger := log.WithField("API Service", name)
	logger.Infof("Waiting for API Service")
	...
	ListWatch := cache.NewListWatchFromClient(c.RESTClient(), "apiservices", "", fields.OneTermEqualSelector("metadata.name", name))
